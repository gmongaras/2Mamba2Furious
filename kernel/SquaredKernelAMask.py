...
def _attn_bwd_dkdvdak(
    ...
    for start_m in tl.range(lo, hi, BLOCK_M, warp_specialize=False):
        ...
        if STAGE_CAUSAL:
            ...
            qkT2 = tl.where(mask, qkT2, 0.0)
            dovT = tl.where(mask, dovT, 0.0)
            A_mask_T = tl.where(mask, A_mask_T, 0.0)

        # Accumulate dk grad
        dk = tl.dot(2 * qkT * dovT * A_mask_T, tl.trans(qT), dk)
            
        # qkT * A_mask is needed for both dv and da
        qkT2_A_mask = qkT2 * A_mask_T
            
        # Accumulate dv grad
        dv = tl.dot(qkT2_A_mask, tl.trans(doT), dv)
        
        # Accumulate dA grad
        dA_k -= tl.sum(qkT2_A_mask * dovT, axis=1)
...
def _attn_bwd_dqdaq(
    ...
    for start_n in tl.range(lo, hi, BLOCK_N, warp_specialize=False):
        ...
        # Accumulate dq
        dq = tl.dot(2 * qkT * dovT_A_mask, tl.trans(kT), dq)
        
        # Compute qkT and accumulate A mask grad
        dA_q += tl.sum(qkT2 * dovT_A_mask, axis=1)
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, warp_specialize, mode, provider, dtype=torch.float16, q=None, k=None, v=None, A_cumsum=None):
    ...
    p = (torch.matmul(q.float(), k.float().transpose(2, 3)) * sm_scale)**2
