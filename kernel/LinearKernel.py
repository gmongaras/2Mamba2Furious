...
def _attn_bwd_dkdv(
                 k, v, # pre loaded
                 desc_q, desc_do, # To load when iterating
                 dk, dv, # To write to
    ...
    for start_m in tl.range(lo, hi, BLOCK_M, warp_specialize=False):
        ...
        if STAGE_CAUSAL:
            ...
            VdoT = tl.where(mask, VdoT, 0.0)
            
        # Accumulate dv grads
        dv = tl.dot(qkT, doT.T, dv)
            
        # Accumulate dk grads
        dk = tl.dot(VdoT, qT.T, dk)
...
def _attn_bwd_dq(
                 q, do, # pre loaded
                 desc_k, desc_v, # To load when iterating
                 dq, # To write to
                 qo_offset_y, offset_y, dtype: tl.constexpr, start_m, qk_scale,  #
                 BLOCK_M: tl.constexpr, HEAD_DIM: tl.constexpr, BLOCK_N: tl.constexpr,  #
                 STAGE_CAUSAL: tl.constexpr, offs_m: tl.constexpr, offs_n: tl.constexpr,  #
...
@triton.jit(debug=DEBUG)
def _attn_bwd(desc_q, desc_k, desc_v, sm_scale,  #
              desc_do,  #
    ...
    # stage 1: causal - on-band - on diag
    dk, dv = _attn_bwd_dkdv(k, v, # pre loaded
                                desc_q_, desc_do_, # To load when iterating
                                dk, dv, # To write to
    ...
    # stage 1: causal - on-band - on diag
    dq = _attn_bwd_dq(q, do, # pre loaded
                                desc_k_, desc_v_, # To load when iterating
                                dq, # To write to
                                qo_offset_y, offset_y, dtype, start_m, qk_scale,  #
...
def test_op(Z, H, N_CTX, HEAD_DIM, causal, warp_specialize, mode, provider, dtype=torch.float16, q=None, k=None, v=None, A_cumsum=None):
    ...
    q = q.half().detach().requires_grad_(True)
    k = k.half().detach().requires_grad_(True)
    v = v.half().detach().requires_grad_(True)
    
    p = torch.matmul(q, k.transpose(2, 3)) * sm_scale
    p = p.masked_fill(~attention_mask.bool(), 0)
    ref_out = torch.matmul(p, v)#.to(ref_dtype)
    if mode == "bwd":
...
if __name__ == "__main__":
    ...
    """
    #"""
    q = torch.load("debug_output/query_states").cuda().half().contiguous().detach().requires_grad_()
    k = torch.load("debug_output/key_states").cuda().half().contiguous().detach().requires_grad_()
    v = torch.load("debug_output/value_states").cuda().half().contiguous().detach().requires_grad_()
    A_cumsum = torch.load("debug_output/A_cumsum").cuda().half().contiguous().detach().requires_grad_()
    test_op(
        q.shape[0],
        q.shape[1],
        q.shape[2],
