...
def _attn_fwd(sm_scale, M, #
    ...
    l_i = tl.where(l_i > 1e-8, l_i, 1e-8)
...
if __name__ == "__main__":
    ...
    q = q[b_idx:b_idx+1,:,:seq_len].contiguous().float().detach().requires_grad_()
    k = torch.load("debug_output/key_states").cuda().float()[b_idx:b_idx+1,:,:seq_len].contiguous().detach().requires_grad_()
    v = torch.load("debug_output/value_states").cuda().float()[b_idx:b_idx+1,:,:seq_len].contiguous().detach().requires_grad_()
    A_cumsum = torch.load("debug_output/A_cumsum").cuda().float()[b_idx:b_idx+1,:,:seq_len].contiguous().detach().requires_grad_()
