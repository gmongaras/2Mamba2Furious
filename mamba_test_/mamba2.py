...
def ssd_(x, A, B, C, chunk_size, initial_states=None, device: Device = None):
    """Structed State Space Duality (SSD) - the core of Mamba-2
    ...
    Source
     ...
     2. https://github.com/state-spaces/mamba/blob/219f03c840d5a44e7d42e4e728134834fddccf45/mamba_ssm/modules/ssd_minimal.py#L34-L78
    """
    A_cumsum = torch.cumsum(A.mT, dim=-1) # (B, H, L)
    causal_mask = ~torch.tril(torch.ones(A.shape[1], A.shape[1]))[None, None, :, :].to(A.device).bool()
    A_mask = (A_cumsum[..., :, None] - A_cumsum[..., None, :]).masked_fill(causal_mask, -torch.inf).exp()

    M = C.permute(0, 2, 1, 3) @ B.permute(0, 2, 3, 1)
    M = M * A_mask
    Y = M @ x.permute(0, 2, 1, 3)
