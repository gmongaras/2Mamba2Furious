...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int, get_taylor_terms=False):
        ...
        min_val = 4
        # sizes = [max(math.ceil(self.head_dim**(1/i)), min_val) for i in range(1, self.order+1)]
        sizes = [self.head_dim if i == 1 else int(self.head_dim**0.5) for i in range(1, self.order+1)]
        # self.A_projs = nn.ParameterList([
        #     (2 * 1/self.head_dim**0.5)*torch.rand(self.head_dim, sizes[i-1]) - 1/self.head_dim**0.5 for i in range(1, self.order+1)
        # ])
        self.A_projs = nn.ParameterList([
            nn.Parameter(torch.empty(self.head_dim, sizes[i - 1]).uniform_(
                -1.0 / math.sqrt(self.head_dim),
                1.0 / math.sqrt(self.head_dim)
            )) for i in range(1, self.order + 1)
        ])
        # self.B_projs = nn.ParameterList([
        #     ((2 * 1/sizes[i-1]**0.5)*torch.rand(sizes[i-1], self.head_dim) -1/sizes[i-1]**0.5).mT for i in range(1, self.order+1)
        # ])
        self.B_projs = nn.ParameterList([
            nn.Parameter(torch.empty(self.head_dim, sizes[i - 1]).uniform_(
                -1.0 / math.sqrt(sizes[i-1]),
                1.0 / math.sqrt(sizes[i-1])
            )) for i in range(1, self.order + 1)
        ])
        ...
        # Output Norm
        self.norm = nn.RMSNorm(config.head_dim)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        def forwrd_gated(query_states, key_states, value_states, attention_mask, A_projs, B_projs, order_coefficients):
            # Accumulate over orders
            attn_weights = 1
            for _i in range(1, self.order+1):
                cos, sin = self.rope_embeddings[_i-1][:, :, :query_states.shape[2]].to(query_states.device)
                Q, K = apply_rotary_pos_emb(query_states @ A_projs[_i-1], key_states @ B_projs[_i-1], cos, sin)
                attn_weights = attn_weights + (
                            order_coefficients[_i-1:_i] *
                            (Q @ K.mT)**_i
                        )
        ...
        attn_output = checkpoint(
            forwrd_gated, query_states, key_states, value_states, attention_mask, self.A_projs, self.B_projs, self.order_coefficients
        )
