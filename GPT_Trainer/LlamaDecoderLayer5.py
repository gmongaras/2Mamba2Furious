...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int, get_taylor_terms=False):
        ...
        if self.in_conv:
            d_conv = 2
            self.conv1d_qk = nn.Conv1d(
                in_channels=self.head_dim**2,
                out_channels=self.head_dim**2,
                bias=True,
                kernel_size=d_conv,
                groups=self.head_dim**2,
                padding=d_conv - 1,
            )
        ...
        if not self.use_exp:
            if self.gamma_domain:
                self.order_coefficients = torch.tensor(self.powers, dtype=torch.float)[:, None, None, None].repeat(1, config.num_attention_heads, 1, 1).float()
                ...
                self.order_coefficients = torch.tensor([0.0 for i in self.powers])[:, None, None, None].repeat(1, config.num_attention_heads, 1, 1).float()
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Used to d oa self kronekcer product on the dimension axis
        def kron(X):
        ...
        def forwrd_gated(query_states, key_states, value_states, attention_mask, order_coefficients, norm, conv1d_weight_qk, conv1d_bias_qk, conv1d_weight_v, conv1d_bias_v):
            # Add heads
            query_states = query_states.view(hidden_shape).transpose(1, 2)
            key_states = key_states.view(hidden_shape).transpose(1, 2)
            value_states = value_states.view(hidden_shape).transpose(1, 2)

            # Kronecker on the queries and keys
            query_states = kron(query_states)
            key_states = kron(key_states)

            # Remove heads
            shp = query_states.shape
            shp_v = value_states.shape
            query_states = query_states.reshape(shp[0]*shp[1], shp[2], shp[3])
            key_states = key_states.reshape(shp[0]*shp[1], shp[2], shp[3])
            value_states = value_states.reshape(shp_v[0]*shp_v[1], shp_v[2], shp_v[3])

            # Apply input convolution
            if self.in_conv:
                query_states = causal_conv1d_fn(
                    x=query_states.transpose(1, 2),
                    weight=rearrange(conv1d_weight_qk, "d 1 w -> d w"),
                    bias=conv1d_bias_qk,
                    activation=None,
                ).transpose(1, 2)
                ...
                value_states = causal_conv1d_fn(
                    ...
                    weight=rearrange(conv1d_weight_v, "d 1 w -> d w"),
                    bias=conv1d_bias_v,
                    activation=None,
                ).transpose(1, 2)

            # Add heads back
            query_states = query_states.reshape(shp)
            key_states = key_states.reshape(shp)
            value_states = value_states.reshape(shp_v)
            ...
            attn_weights = order_coefficients[0] + order_coefficients[1]*(query_states @ key_states.mT * (1/math.sqrt(key_states.shape[-1])))            
        ...
        attn_output = checkpoint(
            forwrd_gated, query_states, key_states, value_states, attention_mask, self.order_coefficients, self.out_norm, self.conv1d_qk.weight, self.conv1d_qk.bias, self.conv1d_v.weight, self.conv1d_v.bias
        )
