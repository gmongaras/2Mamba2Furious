...
class Mamba2_SM(nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        d_model,
        d_state=128,
    ...
    def forward(self, u, seqlen=None, seq_idx=None, cu_seqlens=None, inference_params=None):
        ...
        if self.use_mem_eff_path and inference_params is None:
            ...
            if self.no_dt:
                dt = dt*0 + 1
            B = B.repeat(1, 1, self.nheads // self.ngroups, 1)
            ...
            x = x * dt[..., None]
            if self.A_proj:
                A = A[None, None, :].to(x.dtype) * dt
            
            # (batch, num_heads, seq_len, ssm_dim) --> (batch, seq_len, num_heads, ssm_dim)
            C = C.transpose(1, 2)
            B = B.transpose(1, 2)
            ...
            # Gated
            # M = M.masked_fill(mask, -torch.inf)
            # M = M.exp() / (~mask).sum(-1, keepdim=True)
